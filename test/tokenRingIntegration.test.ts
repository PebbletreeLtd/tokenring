...
describe.each(modes)("tokenRing (%s transport)", (transportMode) => {
    ...
    async function createTestTokenRing(options: {
        ...
        const tr = await new TestingTokenRingDistributor({
            ...
            storage: {
                doTn: (callback) => {
                    return TestingStore.doTn(txn => {
                        return callback({
                            tokenRingRegistration: {
                                get: async (key) => {
                                    return txn.at(TestingStore).get(key);
                                },
                                set: (key, value) => {
                                    txn.at(TestingStore).set(key, value);
                                },
                                clear: (key) => {
                                    txn.at(TestingStore).clear(key);
                                },
                                getRangeAll: async (start, end, options) => {
                                    return txn.at(TestingStore).getRangeAll(start, end, options);
                                },
                            }
                        });
                    })
        ...
        return {
            ...
            TestingStore, // expose the store for direct queries in tests
    ...
    test("tokenRing: server registers itself in WorkflowRingRegistration table", async () => {
        ...
        const { tr, TestingStore } = await createTestTokenRing({ segmentName, capabilities });
    ...
    test("tokenRing: re-registration refreshes the membership entry", async () => {
        ...
        const { tr, TestingStore } = await createTestTokenRing({
            segmentName,
            capabilities,
        ...
        try {
            ...
            const initial = await TestingStore.doTn(async txn => {
                const regs = await txn.at(segmentSubspace).getRangeAllStartsWith({ segment_name: segmentName });
            ...
            const updated = await TestingStore.doTn(async txn => {
                const regs = await txn.at(segmentSubspace).getRangeAllStartsWith({ segment_name: segmentName });
                return regs[0]?.[1]?.last_seen ?? 0;
            });
    ...
    test("tokenRing: works with various issuer_id formats", async () => {
        ...
        const tr = await new TestingTokenRingDistributor({
            ...
            storage: {
                doTn: (callback) => {
                    return TestingStore.doTn(async txn => {
    ...
    test("tokenRing: re-registration preserves extra optional members on the value", async () => {
        ...
        try {
            ...
            const regs = await TestingStore.doTn(async txn => {
                return txn.at(segmentSubspace).getRangeAllStartsWith({ segment_name: segmentName });
            });
            ...
            const before = await TestingStore.doTn(async txn => txn.get(regKey)) as any;
            ...
            const after = await TestingStore.doTn(async txn => txn.get(regKey)) as any;
